...
import { auth, db } from '../firebase'; // Ensure this matches your Firebase config
import { collection, addDoc, doc, getDoc } from 'firebase/firestore';

/**
 * Types defining the structure of data used in the form.
 */
type OccupantInfo = {
  name: string;
  contact: string;
...
/**
 * Props for the AddCondoUnitForm component.
 */
...
/**
 * A form component for adding a new condo unit to a property.
 */
const AddCondoUnitForm: React.FC<CondoUnitFormProps> = ({ propertyId, onUnitSaved }) => {
  ...
  /**
   * Updates the unit state with changes from form inputs.
   */
  ...
  /**
   * Saves the new condo unit to the Firestore database.
   */
  const saveCondoUnit = async () => {
    ...
    try {
      await addDoc(collection(db, `properties/${propertyId}/condoUnits`), { ...unit });
      alert('Condo unit saved successfully!');
      setUnit(initialUnitState); // Reset form
      onUnitSaved();
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        <View>
          <Text>Unit ID <Text style={styles.mandatory}>*</Text></Text>
          <TextInput
            style={styles.input}
        ...
        <View>
          <Text>Size <Text style={styles.mandatory}>*</Text></Text>
          <TextInput
            style={styles.input}
        ...
        <View>
          <Text>Monthly Condo Fees <Text style={styles.mandatory}>*</Text></Text>
          <TextInput
            style={styles.input}
