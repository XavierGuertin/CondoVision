/**
 * NotificationsScreenUser Component
 *
 ...
 *
 * Usage:
 * Import and include `<NotificationsScreenUser />` in your component tree to enable notification management
...
const RequestBox = ({ onRequestSubmit, properties }) => {
  ...
  const [selectedProperty, setSelectedProperty]: any = useState(
    properties[0]?.id || ""
  );
...
const NotificationsScreenUser = () => {
    ...
    const fetchProperties = async () => {
      ...
      if (user) {
        ...
        for (let propertyDoc of querySnapshot.docs) {
          const condoUnitsCollectionRef = collection(
            propertyDoc.ref,
            "condoUnits"
          );
  ...
  const { userRequests, setUserRequests, handleDeleteRequest } =
    useUserRequests();
    ...
    <ScrollView>
      <View>
        ...
        {notifications.map((notification: any) => (
          <View key={notification.id} style={styles.notificationBox}>
            ...
            <TouchableOpacity
              style={styles.notificationBoxButton}
      ...
      {!isRequestBoxVisible && (
        <TouchableOpacity
          ...
          onPress={() => setRequestBoxVisible(true)}
        >
      ...
      {isRequestBoxVisible && (
        <TouchableOpacity
          style={styles.makeRequestButton}
          onPress={() => setRequestBoxVisible(false)}
        >
...
export default NotificationsScreenUser;
